 ...
 */
export function combineRatings(agencies, date) {
  let pairs = [];
  ...
  const valid = pairs.map(([ag, r]) => [ag, normalizeRating(r)]).filter(([, r]) => r);
...
export function applyRatingOverrides(securities, override) {
  ...
  for (const s of securities) {
    ...
    if (!o) continue;
    const combined = combineRatings(o.agencies || o.ratings || o.rating, o.date);
