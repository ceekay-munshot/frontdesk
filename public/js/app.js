...
function tradedNormalLine(isin, curYield, pinned) {
  ...
  // Drill-down: the actual NSE trades behind "usually trades" (the client asked to
  // SEE them). A click pins the tooltip and shows date + traded yield; hover shows
  // a hint. Newest first, capped for display.
  const hist = (tradedRef(isin)?.history || []).filter((h) => isNum(h.y));
  let drill = "";
  if (hist.length) {
    if (pinned) {
...
function nearestBenchmark(curve, tenor) {
  ...
  // `approx`: no traded govt security sits near this tenor (nearest is >1y away),
  // so the govt yield is INTERPOLATED across a gap, not a true matching-maturity
  // read. CCIL's public feed only lists ~5y+ G-Secs + the overnight rate, so
  // 1-4y corporates fall in this gap — the tooltip says so honestly rather than
  // naming a far-off security as if it were the benchmark.
  if (best) best.approx = best.d > 1.0;
  return best;
}
...
function spreadGridHTML(rows, buckets, gridStats) {
  ...
  const body = rows.map((r) => {
    ...
    const cells = buckets.map((bk) => {
      ...
      const tip = JSON.stringify({ kind: "cell", issuer: r.issuer, category: r.category, bucket: bk, spread: c.median, corpY: +c.corpY.toFixed(2), govtY: +c.govtY.toFixed(2), n: c.n, bench: c.bench ? { name: c.bench.name, y: +c.bench.y.toFixed(2), t: c.bench.t, type: c.bench.type, approx: c.bench.approx } : null, accent: bg, isin: c.isin });
  ...
  return `<div class="space-y-4 p-4">
    <section class="rounded-xl border border-slate-100 bg-white/70 p-3">
      <div class="mb-1 flex flex-wrap items-center gap-2">
        ...
        ${c.govtSource === "CCIL" ? `<a href="https://www.ccilindia.com/" target="_blank" rel="noopener noreferrer" class="rounded-full px-2 py-0.5 text-[10px] font-semibold bg-emerald-50 text-emerald-700 hover:underline">CCIL — traded T-bills & G-Secs ↗</a>` : `<span class="rounded-full px-2 py-0.5 text-[10px] font-semibold bg-amber-50 text-amber-700">from desk chat (CCIL unavailable)</span>`}
...
function computeOpportunities() {
  ...
  for (const b of u.bonds) {
    ...
    if (isNum(b.govtSpread) && b.govtSpread >= 100 && b.govtSpread <= OPP_PICKUP_CAP) {
      ...
      o.why = b.bench && b.bench.approx
        ? `Pays ${b.govtSpread} bps over the government curve — but this maturity has no traded govt bond nearby, so the govt yield is interpolated; the "vs Similar bonds" view is the truer read here.`
        : `Pays ${b.govtSpread} bps over ${b.bench ? b.bench.name : "the government curve"} for its maturity — a big extra yield.`;
